...
Public Class clsPettyCash
    ...
    Private Acc_PCM_BatchNo As Integer
    Private Acc_PCM_BaseName As Integer
    Public Property iAcc_PCM_BatchNo() As Integer
        Get
            Return (Acc_PCM_BatchNo)
        End Get
        Set(ByVal Value As Integer)
            Acc_PCM_BatchNo = Value
        End Set
    End Property
    Public Property iAcc_PCM_BaseName() As Integer
        Get
            Return (Acc_PCM_BaseName)
        End Get
        Set(ByVal Value As Integer)
            Acc_PCM_BaseName = Value
        End Set
    End Property
    ...
    Public Function LoadPettyCash(ByVal sNameSpace As String, ByVal iCompID As Integer, ByVal iStatus As Integer, ByVal iYearID As Integer) As DataTable
        ...
        Try
            ...
            sSql = "Select * from Acc_PettyCash_Master where Acc_PCM_YearID=" & iYearID & " And Acc_PCM_CompID =" & iCompID & " "
            If iStatus = 0 Then
    ...
    Public Function GenerateTransactionNo(ByVal sNameSpace As String, ByVal iCompID As Integer, ByVal iYearID As Integer) As String
        ...
        Dim ds As New DataSet
        Try
            iMax = objDBL.SQLExecuteScalar(sNameSpace, "Select isnull(Count(Acc_PCM_ID)+1,1) from Acc_PettyCash_Master Where Acc_PCM_YearID=" & iYearID & " And Acc_PCM_CompID=" & iCompID & " ")
    ...
    Public Function LoadLocations(ByVal sNameSpace As String, ByVal iCompID As Integer, ByVal iParty As Integer) As DataTable
        ...
        End Try
    End Function
    Public Function SavePettyCashDetails(ByVal sNameSpace As String, ByVal iCompID As Integer, objPC As clsPettyCash)
        ...
        Try
            ...
            ObjParam(iParamCount) = New OleDb.OleDbParameter("@ACC_PCM_ZoneID", OleDb.OleDbType.Integer, 4)
            ObjParam(iParamCount).Value = objPC.iACC_PCM_ZoneID
            ObjParam(iParamCount).Direction = ParameterDirection.Input
            iParamCount += 1

            ObjParam(iParamCount) = New OleDb.OleDbParameter("@ACC_PCM_RegionID", OleDb.OleDbType.Integer, 4)
            ObjParam(iParamCount).Value = objPC.iACC_PCM_RegionID
            ObjParam(iParamCount).Direction = ParameterDirection.Input
            iParamCount += 1

            ObjParam(iParamCount) = New OleDb.OleDbParameter("@ACC_PCM_AreaID", OleDb.OleDbType.Integer, 4)
            ObjParam(iParamCount).Value = objPC.iACC_PCM_AreaID
            ObjParam(iParamCount).Direction = ParameterDirection.Input
            iParamCount += 1

            ObjParam(iParamCount) = New OleDb.OleDbParameter("@ACC_PCM_BranchID", OleDb.OleDbType.Integer, 4)
        ...
        End Try
    End Function
    Public Function SaveTransactionDetails(ByVal sNameSpace As String, ByVal iCompID As Integer, objPC As clsPettyCash)
    ...
    '        dtDetails = objDBL.SQLExecuteDataSet(sNameSpace, "Select * From Acc_Transactions_Details Where ATD_Status='A' And ATD_TrType=2 And ATD_BillID=" & iMasId & " And ATD_CompID=" & iCompID & " And ATD_YearID=" & iYearID & " ").Tables(0)
    ...
    '                    sSql = sSql & " Where A.ATD_Status='A' And A.ATD_TrType=2 And A.ATD_BillID=" & iMasId & " And A.ATD_Head=" & iTrAccHead & " And A.ATD_SubGL=" & iTrGLID & " And A.ATD_CompID=" & iCompID & " And A.ATD_YearID=" & iYearID & " And C.ALM_Head=" & iTrHead & ""
    ...
    '                    sSql = sSql & " Where A.ATD_Status='A' And A.ATD_TrType=2 And A.ATD_BillID=" & iMasId & " And A.ATD_Head=" & iTrAccHead & " And A.ATD_GL=" & iTrGLID & " And A.ATD_CompID=" & iCompID & " And A.ATD_YearID=" & iYearID & " And C.ALM_Head=" & iTrHead & " "
    '                    dtValues = objDBL.SQLExecuteDataSet(sNameSpace, sSql).Tables(0)
    '                End If
    ...
    Public Function WriteGLTable(ByVal sNameSpace As String, ByVal iCompID As Integer, ByVal iMasID As Integer, ByVal iyearID As Integer, ByVal iUserID As Integer)
        ...
        Dim iMaxID As Integer
        Try
            sSql = "" : sSql = "Select * From Acc_Transactions_Details Where ATD_Status='A' And ATD_TrType=2 And ATD_BillID=" & iMasID & " And ATD_YearID=" & iyearID & " And ATD_CompID=" & iCompID & " "
            dt = objDBL.SQLExecuteDataSet(sNameSpace, sSql).Tables(0)
            If dt.Rows.Count > 0 Then
                For i = 0 To dt.Rows.Count - 1
