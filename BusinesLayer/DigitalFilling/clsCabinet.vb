...
Public Class clsCabinet
    ...
    Public Property iCBN_PERMISSION() As Integer
        ...
        End Set
    End Property
    Public Function LoadCabinetGrid(ByVal sNameSpace As String, ByVal iDescID As Integer, ByVal sSearchIndex As String) As DataTable
        ...
        dtDisplay.Columns.Add("CBN_SCCount")
        dtDisplay.Columns.Add("CBN_FolCount")
        dtDisplay.Columns.Add("CBN_CRON")
        dtDisplay.Columns.Add("cbn_DelStatus")
        Try
            sSql = "Select CBN_NODE,CBN_NAME,CBN_NOTE,CBN_SCCount,CBN_FolCount,CBN_ParGrp,CBN_CRON,cbn_DelStatus from edt_cabinet where CBN_Parent=-1  And  CBN_PERMISSION <> '1' "
            If iDescID > 0 Then
            ...
            If dr.HasRows Then
                While dr.Read
                    ...
                    End If
                    If IsDBNull(dr("CBN_SCCount")) = False Then
                        dRow("CBN_SCCount") = dr("CBN_SCCount")
                    End If
                    If IsDBNull(dr("CBN_FolCount")) = False Then
                        dRow("CBN_FolCount") = dr("CBN_FolCount")
                    End If
                    If IsDBNull(dr("CBN_CRON")) = False Then
                        dRow("CBN_CRON") = objclsGRACeGeneral.FormatDtForRDBMS(dr("CBN_CRON"), "D")
                    End If
    ...
    Public Function CheckCabName(ByVal sAC As String, ByVal sCabName As String, ByVal iCabID As Integer, Optional ByVal iGrpId As Integer = 0) As Boolean
        ...
        Try
            If (iGrpId = 0) Then
                ...
                sSql = "Select CBN_Name from edt_cabinet where CBN_Name='" & sCabName & "' and  CBN_Node <>" & iCabID & " and CBN_Parent=-1 and (CBN_DelStatus='A' or CBN_DelStatus='W')"
            Else
                'Check cabinet name only for that group
                sSql = "Select CBN_Name from edt_cabinet where CBN_Name='" & sCabName & "' and CBN_ParGrp=" & iGrpId & " and CBN_Node <> " & iCabID & "  and CBN_Parent=-1 and (CBN_DelStatus='A' or CBN_DelStatus='W')"
            End If
        ...
        End Try
    End Function
    Public Function SaveCabDetails(ByVal sNameSpace As String, ByVal objCab As clsCabinet) As Array
        ...
        End Try
    End Function
    Public Sub UpdateCabDetails(ByVal sNameSpace As String, ByVal iCBN_ParId As Integer, ByVal iCBN_NODE As Integer)
        Dim sSql As String, strsql As String, aSql As String
        Try
            'Update both cabinet and sub cabinet department
            sSql = "Update edt_cabinet set CBN_ParGrp='" & iCBN_ParId & "' "
            sSql = sSql & " Where CBN_NODE=' " & iCBN_NODE & "' or CBN_PARENT=' " & iCBN_NODE & "'"
            objDb.SQLExecuteNonQuery(sNameSpace, sSql)

            'Update Sub cabinet count
            strsql = "Update edt_cabinet set CBN_SCCount=(Select count (CBN_Node) from Edt_Cabinet where CBN_Parent=' " & iCBN_NODE & "' and CBN_DelStatus='A') where CBN_NODE=' " & iCBN_NODE & "'"
            objDb.SQLExecuteNonQuery(sNameSpace, strsql)

            'Update folder count
            aSql = "Update edt_cabinet set CBN_FolCount=(select count(Fol_folid) from edt_folder where fol_cabinet in (Select CBN_Node from Edt_Cabinet where CBN_Parent=" & iCBN_NODE & " and CBN_DelStatus='A'))where CBN_NODE=' " & iCBN_NODE & "'"
