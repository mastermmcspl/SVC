...
Partial Class Reports_Viewer_PROFormaHR
    Inherits System.Web.UI.Page
    Private sFormName As String = "Reports_Viewer_PROFormaHR"
    Private objErrorClass As New BusinesLayer.Components.ErrorClass
    Private objclsGeneralFunctions As New clsGeneralFunctions
    Dim objMaster As New clsGeneralMaster
    Dim objGen As New clsFASGeneral
    Dim objGenFun As New clsGeneralFunctions
    Dim objProFormaHR As New clsPROFormaHR
    ...
    Protected Sub loadgrid(ByVal iOrderID As Integer)
        ...
        'Dim sDisplayAmt As String

        Dim intpart As Integer
        Dim decpart As Double
        Try
            'company = objProFormaHR.GetAccessCode(sSession.AccessCode, sSession.AccessCodeID)
            dt0 = objProFormaHR.LoadCompanyDetails(sSession.AccessCode, sSession.AccessCodeID)
            dt1 = objProFormaHR.LoadGridOtherDetails(sSession.AccessCode, sSession.AccessCodeID)
            ...
            Next
            dt2 = objProFormaHR.Party(sSession.AccessCode, sSession.AccessCodeID, sSession.YearID, iOrderID)
            If (dt2.Rows.Count > 0) Then
                partyid = dt2.Rows(0).Item("SPO_PartyName")
                dt3 = objProFormaHR.LoadGridStatutoryReferencesDetails(sSession.AccessCode, sSession.AccessCodeID, partyid)
            ...
            dt4 = objProFormaHR.loadSalesPROFormaDetails(sSession.AccessCode, sSession.AccessCodeID, iOrderID)
            ...
            Duniqe = objProFormaHR.RemoveDublicate(dt5)
            For j = 0 To Duniqe.Rows.Count - 2
                ...
                For i = 0 To dt4.Rows.Count - 2
                    ...
                    If (Duniqe.Rows(j)("Description") = dt4.Rows(i)("Description")) Then
                        If (dt4.Rows(i)("Description") <> "<b>Total</b>") Then
                            ...
                            'If i8Qty > 0 Then
                            '    sBasicAmt = objProFormaHR.CalculateNetAmount(sSession.AccessCode, sSession.AccessCodeID, sSession.YearID, iOrderID, dMRPRate, sPartyCode)
                            '    sNetAmt = i8Qty * sBasicAmt
                            '    sDisplayAmt = sDisplayAmt + sNetAmt
                            'End If
                            'If i10Qty > 0 Then
                            '    sBasicAmt = objProFormaHR.CalculateNetAmount(sSession.AccessCode, sSession.AccessCodeID, sSession.YearID, iOrderID, dMRPRate, sPartyCode)
                            '    sNetAmt = i10Qty * sBasicAmt
                            '    sDisplayAmt = sDisplayAmt + sNetAmt
                            'End If
                ...
                If sPartyCode.StartsWith("C") Then
                    If CstAmnt > 0 Then
                        ...
                        sDisAmt = objProFormaHR.CalculateDiscountAmtMRPCST(sSession.AccessCode, sSession.AccessCodeID, sSession.YearID, iOrderID, dMRPRate, sPartyCode, sItemID, Duniqe.Rows(j)("Discount"))
                        dRow("DisAmt") = String.Format("{0:0.00}", Convert.ToDecimal(qty * sDisAmt))
                    Else
                        Dim sDisAmt As String = ""
                        sDisAmt = objProFormaHR.CalculateDiscountAmtMRP(sSession.AccessCode, sSession.AccessCodeID, sSession.YearID, iOrderID, dMRPRate, sPartyCode, sItemID, Duniqe.Rows(j)("Discount"))
                        dRow("DisAmt") = String.Format("{0:0.00}", Convert.ToDecimal(qty * sDisAmt))
                    End If
                ...
                If sPartyCode.StartsWith("P") Then
                    Dim sDisAmt As String = ""
                    sDisAmt = objProFormaHR.CalculateDiscountAmt(sSession.AccessCode, sSession.AccessCodeID, sSession.YearID, iOrderID, dMRPRate, sPartyCode, Duniqe.Rows(j)("Discount"))
                    dRow("DisAmt") = String.Format("{0:0.00}", Convert.ToDecimal(qty * sDisAmt))
                End If
                ...
                If CstAmnt > 0 Then
                    dRoundTotal = ((((dimGtotal - Duniqe.Rows(j)("TradeDiscount")) + ExceAmnt) + CstAmnt) + Duniqe.Rows(j)("Shipping"))
                Else
                    dRoundTotal = ((((dimGtotal - Duniqe.Rows(j)("TradeDiscount")) + ExceAmnt) + vatAmnt) + Duniqe.Rows(j)("Shipping"))
                End If
                ...
                If sPartyCode.StartsWith("C") Then
                    If CstAmnt > 0 Then
                        ...
                        sNetAmt = objProFormaHR.CalculateNetAmountMRPCST(sSession.AccessCode, sSession.AccessCodeID, sSession.YearID, iOrderID, dMRPRate, sPartyCode, sItemID, Duniqe.Rows(j)("Discount"))
                        dRow("LastColAmount") = String.Format("{0:0.00}", Convert.ToDecimal(qty * sNetAmt))
                    Else
                        Dim sNetAmt As String = ""
                        sNetAmt = objProFormaHR.CalculateNetAmountMRP(sSession.AccessCode, sSession.AccessCodeID, sSession.YearID, iOrderID, dMRPRate, sPartyCode, sItemID, Duniqe.Rows(j)("Discount"))
                        dRow("LastColAmount") = String.Format("{0:0.00}", Convert.ToDecimal(qty * sNetAmt))
                    End If
                ...
                If sPartyCode.StartsWith("P") Then
                    Dim sNetAmt As String = ""
                    sNetAmt = objProFormaHR.CalculateNetAmount(sSession.AccessCode, sSession.AccessCodeID, sSession.YearID, iOrderID, dMRPRate, sPartyCode, Duniqe.Rows(j)("Discount"))
                    dRow("LastColAmount") = String.Format("{0:0.00}", Convert.ToDecimal(qty * sNetAmt))
                End If


                'dRow("LastColAmount") = String.Format("{0:0.00}", Convert.ToDecimal(sDisplayAmt))
                ...
                dRow("TermsConditions") = objProFormaHR.GetTermsConditions(sSession.AccessCode, sSession.AccessCodeID)
            ...
            ReportViewer1.LocalReport.ReportPath = Server.MapPath("~/Reports/rdlc/Sales/PROFormaHR.rdlc")

            Dim VAT As ReportParameter() = New ReportParameter() {New ReportParameter("VAT", UCase(objProFormaHR.GetALLVAT(sSession.AccessCode, sSession.AccessCodeID, sSession.YearID, iOrderID)))}
            ReportViewer1.LocalReport.SetParameters(VAT)

            Dim CST As ReportParameter() = New ReportParameter() {New ReportParameter("CST", UCase(objProFormaHR.GetALLCST(sSession.AccessCode, sSession.AccessCodeID, sSession.YearID, iOrderID)))}
            ...
            dtPrint = objProFormaHR.GetPrintData(sSession.AccessCode, sSession.AccessCodeID, "S")
            ...
            'dtV = objProFormaHR.GetVATBifercation(sSession.AccessCode, sSession.AccessCodeID, sSession.YearID, iOrderID)
            ...
            'dtC = objProFormaHR.GetCSTBifercation(sSession.AccessCode, sSession.AccessCodeID, sSession.YearID, iOrderID)
