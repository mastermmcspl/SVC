...
Partial Class Masters_FrmLgstRouteMasterDashboard
    Inherits System.Web.UI.Page
    Private sFormName As String = "Masters/RouteMasterDashboard"
    ...
    Dim objDPM As New clsDieselPumpMaster
    ...
    Public Sub BindSODetails(ByVal iPageIndex As Integer, ByVal iStatus As Integer)
        ...
        Try
            ...
            Else
                imgbtnWaiting.Visible = False : imgbtnDeActivate.Visible = False : imgbtnActivate.Visible = False
            End If
            dt = objRM.LoadRouteDetails(sSession.AccessCode, sSession.AccessCodeID, sSession.YearID, iStatus)
            dgRoute.DataSource = dt
            dgRoute.DataBind()
        Catch ex As Exception
            lblError.Text = objErrorClass.GetErrorMessages(sSession.AccessCode, ex.Message)
        ...
        End Try
    End Sub
    Private Sub dgRoute_PreRender(sender As Object, e As EventArgs) Handles dgRoute.PreRender
        Dim dt As New DataTable
        Try
            If dgRoute.Rows.Count > 0 Then
                dgRoute.UseAccessibleHeader = True
                dgRoute.HeaderRow.TableSection = TableRowSection.TableHeader
                dgRoute.FooterRow.TableSection = TableRowSection.TableFooter
            End If
        Catch ex As Exception
            lblError.Text = objErrorClass.GetErrorMessages(sSession.AccessCode, ex.Message)
            Components.AppException.LogError(sSession.AccessCode, ex.Message, sFormName, "dgRoute_PreRender")
        End Try
    End Sub
    Private Sub dgRoute_RowDataBound(sender As Object, e As GridViewRowEventArgs) Handles dgRoute.RowDataBound
        ...
        Try
            If e.Row.RowType = DataControlRowType.DataRow Then
                ...
                dgRoute.Columns(0).Visible = True : dgRoute.Columns(6).Visible = False : dgRoute.Columns(7).Visible = True

                If ddlStatus.SelectedIndex = 0 Then
                    imgbtnStatus.ImageUrl = "~/Images/DeActivate16.png" : imgbtnStatus.ToolTip = "De-Activate"
                    If sCMAD = "YES" Then
                        dgRoute.Columns(6).Visible = True
                    End If
                    'If sCMSave = "YES" Then
                ...
                If ddlStatus.SelectedIndex = 3 Then
                    dgRoute.Columns(6).Visible = False : dgRoute.Columns(7).Visible = False
                End If
            End If
        Catch ex As Exception
            lblError.Text = objErrorClass.GetErrorMessages(sSession.AccessCode, ex.Message)
            Components.AppException.LogError(sSession.AccessCode, ex.Message, sFormName, "dgRoute_RowDataBound")
        End Try
    End Sub
    Private Sub dgRoute_RowCommand(sender As Object, e As GridViewCommandEventArgs) Handles dgRoute.RowCommand
        ...
        Try
            ...
            If e.CommandName.Equals("Edit") Then
                ...
                End If
                oMasterID = HttpUtility.UrlEncode(objGen.EncryptQueryString(Val(lblDescID.Text)))
                Response.Redirect(String.Format("~/Masters/FrmLgstRouteMaster.aspx?StatusID={0}&PID={1}", oStatusID, oMasterID), False) 'GeneralMasterDetails
            End If
            If e.CommandName.Equals("Status") Then
                If ddlStatus.SelectedIndex = 0 Then
                    objRM.UpdateRouteMasterStatus(sSession.AccessCode, sSession.AccessCodeID, lblDescID.Text, "D", sSession.UserID, sSession.IPAddress, sSession.YearID)
                ...
                End If
                If ddlStatus.SelectedIndex = 1 Then
                    objRM.UpdateRouteMasterStatus(sSession.AccessCode, sSession.AccessCodeID, lblDescID.Text, "A", sSession.UserID, sSession.IPAddress, sSession.YearID)
                    lblPaymentMasterValidationMsg.Text = "Successfully Activated." : lblError.Text = "Successfully Activated."
                    ScriptManager.RegisterStartupScript(Me, Me.GetType(), "Modal", "$('#divMsgType').addClass('alert alert-success');$('#ModalPaymentValidation').modal('show');", True)
                End If
                If ddlStatus.SelectedIndex = 2 Then
                    objRM.UpdateRouteMasterStatus(sSession.AccessCode, sSession.AccessCodeID, lblDescID.Text, "W", sSession.UserID, sSession.IPAddress, sSession.YearID)
        ...
        Catch ex As Exception
            lblError.Text = objErrorClass.GetErrorMessages(sSession.AccessCode, ex.Message)
            Components.AppException.LogError(sSession.AccessCode, ex.Message, sFormName, "dgRoute_RowCommand")
        End Try
    End Sub
    Private Sub imgbtnAdd_Click(sender As Object, e As ImageClickEventArgs) Handles imgbtnAdd.Click
        ...
        Try
            ...
            End If
            Response.Redirect(String.Format("~/Masters/FrmLgstRouteMaster.aspx?StatusID={0}&PID={1}", oStatusID, oMasterName), False)
        Catch ex As Exception
            lblError.Text = objErrorClass.GetErrorMessages(sSession.AccessCode, ex.Message)
    ...
    Private Sub imgbtnActivate_Click(sender As Object, e As ImageClickEventArgs) Handles imgbtnActivate.Click
        ...
        Try
            ...
            If dgRoute.Rows.Count = 0 Then
                ...
                Exit Sub
            End If
            For i = 0 To dgRoute.Rows.Count - 1
                chkSelect = dgRoute.Rows(i).FindControl("chkSelect")
            ...
            If iCount = 0 Then
                ...
                Exit Sub
            End If
NextSave:   For i = 0 To dgRoute.Rows.Count - 1
                chkSelect = dgRoute.Rows(i).FindControl("chkSelect")
                lblDescID = dgRoute.Rows(i).FindControl("lblDescID")
                If chkSelect.Checked = True Then
                    objRM.UpdateRouteMasterStatusWhole(sSession.AccessCode, sSession.AccessCodeID, sSession.YearID, "D", "A", sSession.UserID, sSession.IPAddress, lblDescID.Text)
...
NextSave:   For i = 0 To dgRoute.Rows.Count - 1
                ...
                lblDescID = dgRoute.Rows(i).FindControl("lblDescID")
                If chkSelect.Checked = True Then
                    objRM.UpdateRouteMasterStatusWhole(sSession.AccessCode, sSession.AccessCodeID, sSession.YearID, "A", "D", sSession.UserID, sSession.IPAddress, lblDescID.Text)
...
NextSave:   For i = 0 To dgRoute.Rows.Count - 1
                ...
                lblDescID = dgRoute.Rows(i).FindControl("lblDescID")
                If chkSelect.Checked = True Then
                    objRM.UpdateRouteMasterStatusWhole(sSession.AccessCode, sSession.AccessCodeID, sSession.YearID, "W", "A", sSession.UserID, sSession.IPAddress, lblDescID.Text)
    ...
    Protected Sub chkSelectAll_CheckedChanged(sender As Object, e As EventArgs)
        ...
        Try
            ...
            If chkAll.Checked = True Then
                For iIndx = 0 To dgRoute.Rows.Count - 1
                    chkField = dgRoute.Rows(iIndx).FindControl("chkSelect")
        ...
        End Try
    End Sub
    Private Sub dgRoute_RowEditing(sender As Object, e As GridViewEditEventArgs) Handles dgRoute.RowEditing
    ...
    Private Sub lnkbtnExcel_Click(sender As Object, e As EventArgs) Handles lnkbtnExcel.Click
        ...
        Dim dt As New DataTable
        Try
            dt = objRM.LoadRouteDetails(sSession.AccessCode, sSession.AccessCodeID, sSession.YearID, ddlStatus.SelectedIndex)
