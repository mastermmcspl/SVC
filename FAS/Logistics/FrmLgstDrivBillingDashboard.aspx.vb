...
Partial Class Logistics_FrmLgstDrivBillingDashboard
    ...
    Public Sub BindSODetails(ByVal iPageIndex As Integer, ByVal iStatus As Integer)
        ...
        Try
            ...
            Else
                imgbtnWaiting.Visible = False : imgbtnDeActivate.Visible = False : imgbtnActivate.Visible = False
            End If
            dt = objLDB.LoadDriverBillingDetailsDashboard(sSession.AccessCode, sSession.AccessCodeID, sSession.YearID, iStatus)
            dgDriverBillingDashboard.DataSource = dt
            dgDriverBillingDashboard.DataBind()
        Catch ex As Exception
            lblError.Text = objErrorClass.GetErrorMessages(sSession.AccessCode, ex.Message)
            Components.AppException.LogError(sSession.AccessCode, ex.Message, sFormName, "BindSODetails")
        End Try
    End Sub
    Private Sub dgDriverBillingDashboard_PreRender(sender As Object, e As EventArgs) Handles dgDriverBillingDashboard.PreRender
        Dim dt As New DataTable
        Try
            If dgDriverBillingDashboard.Rows.Count > 0 Then
                dgDriverBillingDashboard.UseAccessibleHeader = True
                dgDriverBillingDashboard.HeaderRow.TableSection = TableRowSection.TableHeader
                dgDriverBillingDashboard.FooterRow.TableSection = TableRowSection.TableFooter
            End If
        Catch ex As Exception
            lblError.Text = objErrorClass.GetErrorMessages(sSession.AccessCode, ex.Message)
            Components.AppException.LogError(sSession.AccessCode, ex.Message, sFormName, "dgDriverBillingDashboard_PreRender")
        End Try
    End Sub
    Private Sub dgDriverBillingDashboard_RowDataBound(sender As Object, e As GridViewRowEventArgs) Handles dgDriverBillingDashboard.RowDataBound
        ...
        Try
            If e.Row.RowType = DataControlRowType.DataRow Then
                ...
                dgDriverBillingDashboard.Columns(0).Visible = True : dgDriverBillingDashboard.Columns(7).Visible = False : dgDriverBillingDashboard.Columns(8).Visible = True

                If ddlStatus.SelectedIndex = 0 Then
                    imgbtnStatus.ImageUrl = "~/Images/DeActivate16.png" : imgbtnStatus.ToolTip = "De-Activate"
                    If sCMAD = "YES" Then
                        dgDriverBillingDashboard.Columns(7).Visible = True
                    End If
                    'If sCMSave = "YES" Then
                ...
                If ddlStatus.SelectedIndex = 3 Then
                    dgDriverBillingDashboard.Columns(7).Visible = False : dgDriverBillingDashboard.Columns(8).Visible = False
                End If
            End If
        Catch ex As Exception
            lblError.Text = objErrorClass.GetErrorMessages(sSession.AccessCode, ex.Message)
        ...
        End Try
    End Sub
    Private Sub dgDriverBillingDashboard_RowCommand(sender As Object, e As GridViewCommandEventArgs) Handles dgDriverBillingDashboard.RowCommand
        ...
        Try
            ...
            If e.CommandName.Equals("Edit") Then
                ...
                End If
                oMasterID = HttpUtility.UrlEncode(objGen.EncryptQueryString(Val(lblDescID.Text)))
                Response.Redirect(String.Format("~/Logistics/FrmLgstDriverBilling.aspx?StatusID={0}&PID={1}", oStatusID, oMasterID), False) 'GeneralMasterDetails
            End If
            If e.CommandName.Equals("Status") Then
                If ddlStatus.SelectedIndex = 0 Then
                    objLDB.UpdateDriverBillingMasterStatus(sSession.AccessCode, sSession.AccessCodeID, lblDescID.Text, "D", sSession.UserID, sSession.IPAddress, sSession.YearID)
                ...
                End If
                If ddlStatus.SelectedIndex = 1 Then
                    objLDB.UpdateDriverBillingMasterStatus(sSession.AccessCode, sSession.AccessCodeID, lblDescID.Text, "A", sSession.UserID, sSession.IPAddress, sSession.YearID)
                    lblPaymentMasterValidationMsg.Text = "Successfully Activated." : lblError.Text = "Successfully Activated."
                    ScriptManager.RegisterStartupScript(Me, Me.GetType(), "Modal", "$('#divMsgType').addClass('alert alert-success');$('#ModalPaymentValidation').modal('show');", True)
                End If
                If ddlStatus.SelectedIndex = 2 Then
                    objLDB.UpdateDriverBillingMasterStatus(sSession.AccessCode, sSession.AccessCodeID, lblDescID.Text, "W", sSession.UserID, sSession.IPAddress, sSession.YearID)
        ...
        Catch ex As Exception
            lblError.Text = objErrorClass.GetErrorMessages(sSession.AccessCode, ex.Message)
            Components.AppException.LogError(sSession.AccessCode, ex.Message, sFormName, "dgDriverBillingDashboard_RowCommand")
        End Try
    End Sub
    ...
    Private Sub imgbtnActivate_Click(sender As Object, e As ImageClickEventArgs) Handles imgbtnActivate.Click
        ...
        Try
            ...
            If dgDriverBillingDashboard.Rows.Count = 0 Then
                ...
                Exit Sub
            End If
            For i = 0 To dgDriverBillingDashboard.Rows.Count - 1
                chkSelect = dgDriverBillingDashboard.Rows(i).FindControl("chkSelect")
            ...
            If iCount = 0 Then
                ...
                Exit Sub
            End If
NextSave:   For i = 0 To dgDriverBillingDashboard.Rows.Count - 1
                chkSelect = dgDriverBillingDashboard.Rows(i).FindControl("chkSelect")
                lblDescID = dgDriverBillingDashboard.Rows(i).FindControl("lblDescID")
                If chkSelect.Checked = True Then
                    objLDB.UpdateDriverBillingStatusWhole(sSession.AccessCode, sSession.AccessCodeID, sSession.YearID, "D", "A", sSession.UserID, sSession.IPAddress, lblDescID.Text)
...
NextSave:   For i = 0 To dgDriverBillingDashboard.Rows.Count - 1
                ...
                lblDescID = dgDriverBillingDashboard.Rows(i).FindControl("lblDescID")
                If chkSelect.Checked = True Then
                    objLDB.UpdateDriverBillingStatusWhole(sSession.AccessCode, sSession.AccessCodeID, sSession.YearID, "A", "D", sSession.UserID, sSession.IPAddress, lblDescID.Text)
...
NextSave:   For i = 0 To dgDriverBillingDashboard.Rows.Count - 1
                ...
                lblDescID = dgDriverBillingDashboard.Rows(i).FindControl("lblDescID")
                If chkSelect.Checked = True Then
                    objLDB.UpdateDriverBillingStatusWhole(sSession.AccessCode, sSession.AccessCodeID, sSession.YearID, "W", "A", sSession.UserID, sSession.IPAddress, lblDescID.Text)
    ...
    Private Sub lnkbtnExcel_Click(sender As Object, e As EventArgs) Handles lnkbtnExcel.Click
        ...
        Dim dt As New DataTable
        Try
            dt = objLDB.LoadDriverBillingDetailsDashboard(sSession.AccessCode, sSession.AccessCodeID, sSession.YearID, ddlStatus.SelectedIndex)
    ...
    Protected Sub chkSelectAll_CheckedChanged(sender As Object, e As EventArgs)
        ...
        Try
            ...
            If chkAll.Checked = True Then
                For iIndx = 0 To dgDriverBillingDashboard.Rows.Count - 1
                    chkField = dgDriverBillingDashboard.Rows(iIndx).FindControl("chkSelect")
    ...
    Private Sub dgDriverBillingDashboard_RowEditing(sender As Object, e As GridViewEditEventArgs) Handles dgDriverBillingDashboard.RowEditing
